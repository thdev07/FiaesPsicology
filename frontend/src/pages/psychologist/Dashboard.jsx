...
import { LayoutDashboard, CalendarDays, Users, Clock, FileText, CheckCircle, Wallet, TrendingUp } from 'lucide-react';
...
export default function PsychologistDashboard() {
  ...
  return (
    <motion.div {...pageAnim}>
      ...
      {/* Próximas sessões */}
      <div style={{ background: '#fff', borderRadius: 8, boxShadow: '0 1px 3px rgba(0,0,0,.08)', overflow: 'hidden', marginBottom: 24 }}>
      ...
      <div style={{ background: '#fff', borderRadius: 8, boxShadow: '0 1px 3px rgba(0,0,0,.08)', overflow: 'hidden' }}>
        <div style={{ padding: '1rem 1.25rem', borderBottom: '1px solid #f3f4f6', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          <span style={{ fontWeight: 700, color: '#374151', fontSize: '0.95rem' }}>Meus repasses</span>
        </div>
        {repasse.list.length === 0 ? (
          <div style={{ padding: 32, color: '#9ca3af', textAlign: 'center' }}>Nenhum repasse registrado.</div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: '#f9fafb' }}>
            ...
            <tbody>
              {repasse.list.map((t) => (
                <tr key={t.id} style={{ borderTop: '1px solid #f3f4f6' }}>
                  <td style={{ padding: '0.75rem 1rem', color: '#111827', fontWeight: 500 }}>{t.appointments?.patients?.nome ?? '—'}</td>
                  <td style={{ padding: '0.75rem 1rem', color: '#374151', fontSize: '0.875rem' }}>
                    {t.appointments?.data ? new Date(t.appointments.data + 'T00:00:00').toLocaleDateString('pt-BR') : '—'}
                  </td>
                  <td style={{ padding: '0.75rem 1rem', color: '#374151', fontSize: '0.875rem' }}>{t.appointments?.hora?.slice(0, 5) ?? '—'}</td>
                  <td style={{ padding: '0.75rem 1rem', fontWeight: 700, color: '#111827' }}>{fmt(t.valor)}</td>
                  <td style={{ padding: '0.75rem 1rem' }}>
